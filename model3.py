...
class SRResNet(nn.Module):
    def __init__(
        ...
        out_channels: int = 3,
        channels: int = 64,
        trunk_blocks: int = 6,          # 主分支残差块数
        branch_blocks: int = 5,        # 多尺度分支残差块数
        num_rcb: int = 16,
        upscale: int = 4,
        upscale1: int = 2,
        upscale2: int =4
        ...
        # 1/4尺度分支
        self.scale_branch4 = nn.Sequential(
            *[_ResidualConvBlock(channels) 
              for _ in range(branch_blocks)],
        )
        # ----------------------------
        ...
        self.conv2 = nn.Sequential(
            DeformConv2d(channels, channels, 3, padding=1),
            nn.BatchNorm2d(channels),
        )
        
        self.attn = DeformableAttention(
            dim = channels,              
            dim_head = max(16, channels//8),  
            heads = 8,
            downsample_factor = 4,
            offset_scale = 4,
        ...
        # Upsampling
        upsampling1 = []
        for _ in range(int(math.log2(upscale1))):
            upsampling1.append(_UpsampleBlock(channels, 2))
        self.upsampling1 = nn.Sequential(*upsampling1)
        
        upsampling2 = []
        for _ in range(int(math.log2(upscale2))):
            upsampling2.append(_UpsampleBlock(channels, 2))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        identity3 = nn.MaxPool2d(kernel_size=2)(identity)
        identity4 = nn.MaxPool2d(kernel_size=2)(identity3)
        scale4_out = self.scale_branch4(identity4)  # 1/4尺度输出
        x1 = self.upsampling1(scale4_out)  # 合并于1/2尺度
        x2 = self.upsampling2(scale4_out)  # 合并于主尺度

        identity1 = torch.cat([identity3, x1], dim=1)
        scale2_out = self.scale_branch2(identity1)  # 1/2尺度输出
        x3 = self.upsampling1(scale2_out)  # 合并于主尺度

        identity2 = torch.cat([identity, x2, x3], dim=1)  # 主尺度输入
        main_out = self.main_branch(identity2)  # 主尺度输出

        scale4_out = self.attn(scale4_out)  # 1/4尺度输出经过注意力
        scale4_out1 = self.upsampling1(scale4_out)  # 合并于1/2尺度输出
        scale4_out2 = self.upsampling2(scale4_out)  # 合并于主尺度

        scale2_out = torch.cat([scale2_out, scale4_out1], dim=1)  # 1/2尺度合并
        scale2_out = self.attn(scale2_out)  # 1/2尺度经过注意力
        scale2_out1 = self.upsampling1(scale2_out)  # 合并于主尺度

        main_out = torch.cat([main_out, scale4_out2, scale2_out1], dim=1)  # 主尺度concat
        main_out = self.attn(main_out)  # 主尺度经过注意力

        scale24_out = torch.cat([scale2_out, scale4_out1], dim=1)  # 注意力输出后融合特征
        scale24_out = self.upsampling1(scale24_out)
        main_out = torch.cat([main_out, scale24_out], dim=1)

        # 残差连接
        x = torch.cat([self.conv2(main_out), identity], dim=1)
...
class _ResidualConvBlockWithAttention(nn.Module):
